    ...
    <div className={activo ? styled.card : styled.card2}>
      {activo ? ( // Si el plato está activo, mostrar el botón de eliminar con 'X', de lo contrario, mostrar 'A'
        <div className={styled.deleteContainer}>
          <button className={styled.delete} onClick={() => handleDelete(id)}>
            X
          </button>
        </div>
      ) : (
        <div className={styled.deleteContainer}>
          <button className={styled.delete} onClick={() => handleActive(id)}>
            ACTIVAR PLATO
          </button>
        </div>
      )}
      ...
      <div className={Style.conteinerName}>
        ...
        <div className={Style.calorias}>
          {" "}
          <p className={Style.caloriastexto}>
            {" "}
            {kilocalorias} kilocalorias | {weight}g grasas | {carbohidratos}g
            carbohidratos
          </p>
        </div>
      </div>
